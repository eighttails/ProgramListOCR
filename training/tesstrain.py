...
def main():
    ...
    phase_I_generate_image(ctx, par_factor=psutil.cpu_count(logical=True))
    phase_UP_generate_unicharset(ctx)

    if ctx.linedata:
        phase_E_extract_features(ctx, ["lstm.train"], "lstmf", par_factor=psutil.cpu_count(logical=True))
        make_lstmdata(ctx)
