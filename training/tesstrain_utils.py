...
def run_command(cmd, *args, env=None, retry_count=5, exit_on_error=True):
    ...
    if proc.returncode == 0:
        ...
        if retry_count <= 0:
            ...
            time.sleep(10)
            run_command(cmd, *args, env=env, retry_count=retry_count)
...
def initialize_fontconfig(ctx):
    ...
    check_file_readable(str(pathlib.Path(ctx.font_config_cache) / "fonts.conf"))

def make_fontname(font):
    return font.replace(" ", "_").replace(",", "")


def make_outbase(ctx, fontname, exposure, char_spacing, leading):
    return pathlib.Path(ctx.training_dir) / f"{ctx.lang_code}.{fontname}.e{exposure}.s{char_spacing}.l{leading}"
...
def generate_font_image(ctx, font, exposure, char_spacing, leading):
    log.info(f"Rendering using {font}")
    fontname = make_fontname(font)
    outbase = make_outbase(ctx, fontname, exposure, char_spacing, leading)
    ...
    return f"{font}-{exposure}-{char_spacing}"
...
def phase_I_generate_image(ctx, par_factor=None):
    ...
    with tqdm(
            total=len(ctx.fonts)*len(ctx.exposures)*len(ctx.char_spacings)*len(ctx.leadings)
    ) as pbar, concurrent.futures.ThreadPoolExecutor(max_workers=par_factor) as executor:
        futures = []
        for leading in ctx.leadings:
            for exposure in ctx.exposures:
                for char_spacing in ctx.char_spacings:
                    for font in ctx.fonts:
                        futures.append(executor.submit(generate_font_image, ctx, font, exposure, char_spacing, leading))
                        time.sleep(1)
...
def phase_E_extract_features(ctx, box_config, ext, par_factor=2):
    log.info(f"=== Phase E: Generating {ext} files ===")

    img_files = list(pathlib.Path(ctx.training_dir).glob("*.tif"))
    log.debug(img_files)
