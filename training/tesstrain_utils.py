...
def make_outbase(ctx, fontname, exposure, char_spacing):
    return pathlib.Path(ctx.training_dir) / f"{ctx.lang_code}.{fontname}.exp{exposure}.spc{char_spacing}"
...
def generate_font_image(ctx, font, exposure, char_spacing):
    ...
    outbase = make_outbase(ctx, fontname, exposure, char_spacing)
...
def phase_I_generate_image(ctx, par_factor=None):
    ...
    with tqdm(
            total=len(ctx.fonts)*len(ctx.exposures)*len(ctx.char_spacings)
        ...
        for exposure in ctx.exposures:
            for char_spacing in ctx.char_spacings:
                for font in ctx.fonts:
                    futures.append(executor.submit(generate_font_image, ctx, font, exposure, char_spacing))
                    time.sleep(1)
